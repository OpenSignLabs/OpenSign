...
function Header({
  ...
  const handleDeclinePdfAlert = async () => {
    ...
    declineHandler(currentDecline);
    // setIsDecline(currentDecline);
  };
  const handleOpen = () => setOpenTermsModal(true);
      ...
      <>
        <p
          dangerouslySetInnerHTML={{
            __html: `
            ${replacePlaceholders(t("agreement.intro"), variables)} 
            ${t("agreement.legalClause")} 
            ${replacePlaceholders(t("agreement.rights"), variables)}
          `
          }}
        />
        <br />
        ...
        <p>
          ...
          <a
            ...
            aria-label={`Open ${t("link.label")} (PDF)`}
          >
            {t("link.label")}
          </a>
        </p>
  ...
  return (
    <div className="flex py-[5px]">
              ...
              <div className="flex" data-tut="reactourFifth">
                ...
                >
                  <p style={{ fontSize: "12px" }}>
                    By selecting <strong>{t("start")}</strong>, you agree to the{" "}
                    <span
                      onClick={handleOpen}
                      style={{
                        color: "blue",
                ...
                <div className="op-btn op-btn-ghost op-btn-sm mr-[3px]"></div>
                {/* {!templateId && (
                  <button
                    onClick={() => {
                      console.log('\n----window.history.go(-2)---',window.history);
                      window.history.go(-2);
                    }}
                    type="button"
                    className="op-btn op-btn-ghost op-btn-sm mr-[3px]"
      ...
      >
        <div className="p-3 md:p-5 text-[13px] md:text-base text-left text-base-content">
          <div
            className="overflow-y-auto max-h-60 px-3"
            style={{ maxHeight: "300px", margin: "5px" }}
          >
            {getFormattedText()}
          </div>
          <div style={{ textAlign: "center" }}>
            {!isAccepted && (
              <button
                ...
                type="button"
                className="op-btn op-btn-primary text-center op-btn-sm mr-[3px] shadow"
              >
                {t("accept")}
              </button>
            )}
