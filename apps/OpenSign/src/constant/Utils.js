...
export const signPdfFun = async (
  ...
  try {
    ...
    let getSignature, getInitials;
    for (let item of widgets) {
      if (!getSignature) {
        const typeExist = item.pos.some((data) => data?.type);
        if (typeExist) {
          getSignature = item.pos.find((data) => data?.type === "signature");
          getInitials = item.pos.find((data) => data?.type === "initials");
        } else {
    ...
    const convertToBase64IfUrl = async (image) => {
      if (image.includes("https")) {
        try {
          return await fetchImageBase64(image);
        } catch (e) {
    ...
    const processSignatureAndInitials = async (getSignature, getInitials) => {
      try {
        // Initialize result variables
        let suffixBase64Sign = null;
        let suffixBase64Initials = null;

        // Process signature if getSignature and SignUrl are defined
        if (getSignature?.SignUrl) {
          const base64Sign = await convertToBase64IfUrl(getSignature.SignUrl);
          const resizedSign = await changeImageWH(base64Sign);
          suffixBase64Sign = resizedSign?.split(",").pop();
        }

        // Process initials if getInitials and SignUrl are defined
        if (getInitials?.SignUrl) {
          const base64Initials = await convertToBase64IfUrl(
            getInitials.SignUrl
          );
          const resizedInitials = await changeImageWH(base64Initials);
          suffixBase64Initials = resizedInitials?.split(",").pop();
        }

        return { suffixBase64Sign, suffixBase64Initials };
      } catch (error) {
        console.error("Error processing signatures:", error.message);
        return { suffixBase64Sign: null, suffixBase64Initials: null };
      }
    };

    const { suffixBase64Sign, suffixBase64Initials } =
      await processSignatureAndInitials(getSignature, getInitials);
    ...
    const params = {
      ...
      signature: suffixBase64Sign,
      initials: suffixBase64Initials,
      signerPos: signerPos,
...
const calculateFontSize = (position, containerScale, signyourself) => {
  const font = position?.options?.fontSize || 8;
...
export const contractDocument = async (documentId, contactId) => {
  const data = { docId: documentId, contactId: contactId };
...
export function getFormattedLocalTime(showTimezone = true) {
  const now = new Date();

  // Check if we only need the timezone abbreviation
  if (showTimezone === "timezoneOnly") {
  ...
  const options = {
    timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone, // Get the local timezone automatically
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
  ...
  const timezoneAbbr = Intl.DateTimeFormat("en", { timeZoneName: "short" })
    .formatToParts(now)
    .find((part) => part.type === "timeZoneName").value;
  ...
  if (showTimezone) {
    return `${formattedDate}T${formattedTime} (${timezoneAbbr})`;
  }
