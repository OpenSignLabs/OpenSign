...
function PdfRequestFiles(props) {
  ...
  const [signTypeValue, setSignTypeValue] = useState("");
  const [isAccepted, setIsAccepted] = useState(false);
  const [openTermsModal, setOpenTermsModal] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const prevStep = useRef(null);
  const redirectURL = useRef(null);
  const docUniqueId = useRef(null);
  const isSignerInOffice = useRef({ signerOne: false, signerTwo: false });
  ...
  useEffect(() => {
    if (!signerObjectId || !Array.isArray(signerPos) || signerPos.length === 0)
      return;

    const matchedSigner = signerPos.find(
      ({ signerObjId }) => signerObjId === signerObjectId
    );

    if (!matchedSigner) return;

    const isInsured = matchedSigner.Role === t("insured");
    const signerOfficeState = isInsured
      ? isSignerInOffice.current.signerOne
      : isSignerInOffice.current.signerTwo;

    // if (isCompleted.isModal && signerOfficeState) {
    //   const timeout = setTimeout(() => {
    //     // Remove current page from history so user can't go back
    //     window.history.replaceState(null, "", window.location.href);

    //     if (redirectURL.current != null) {
    //       // Redirect to new URL
  ...
  const changeSignVal = (value) => {
    setSignTypeValue(value);
  };
  ...
  const getDocumentDetails = async (docId, isNextUser) => {
    try {
      ...
      const lastUrlValue = parts[parts.length - 1];
      //getting document details
      const documentData = await contractDocument(docId, lastUrlValue);
      if (documentData && documentData.length > 0) {
        ...
        const signer1OTPRequired = documentData[0].Signer1OTPRequired;
        const signer2OTPRequired = documentData[0].Signer2OTPRequired;
        const docRedirectURL = documentData[0].RedirectURL;
        const isSignerOneInOffice = documentData[0].IsSignerOneInOffice;
        const isSignerTwoInOffice = documentData[0].IsSignerTwoInOffice;
        redirectURL.current = docRedirectURL;
        docUniqueId.current = documentData[0].DocUniqueId;
        isSignerInOffice.current.signerOne = isSignerOneInOffice;
        isSignerInOffice.current.signerTwo = isSignerTwoInOffice;
          ...
          if (
            ...
            const params = {
              ...
              body: {
                ...
                userTimezone: getFormattedLocalTime("timezoneOnly")
              }
            };
        ...
        if (
          ...
          const isEnableOTP =
            getSigners[0].objectId == lastUrlValue
              ? signer1OTPRequired
              : signer2OTPRequired;
          if (!isEnableOTP) {
            try {
  ...
  const declineHandler = async (obj) => {
    const isNotCancelled = await checkIfDocNotCancelled(false);
  ...
  const handleAccept = async () => {
    ...
    const additionalInfo = pdfDetails[0]?.AdditionalUserInfo || [];
    const signer1OTPRequired = pdfDetails[0]?.Signer1OTPRequired;
    const signer2OTPRequired = pdfDetails[0]?.Signer2OTPRequired;
    const updatedUser = additionalInfo.find(
      (user) => user.signerObjId === signerObjectId
    );

    if (updatedUser) {
      updatedUser.termsCond = t("agreement-msg");
      updatedUser.termsCondAcceptedOn = new Date().toISOString();
      updatedUser.userTimezone = getFormattedLocalTime("timezoneOnly");
      updatedUser.otpRequired =
        additionalInfo[0].signerObjId == signerObjectId
          ? signer1OTPRequired
          : signer2OTPRequired;

      // Use setState to update pdfDetails
      setPdfDetails((prevPdfDetails) => {
        return [
          {
            ...prevPdfDetails[0],
            AdditionalUserInfo: additionalInfo.map((user) =>
              user.signerObjId === signerObjectId ? updatedUser : user
            )
          },
          ...prevPdfDetails.slice(1)
        ];
      });
    }
    setIsAccepted(true);
    ...
    setTimeout(() => {
      embedWidgetsData(null, true);
    }, 200);
  };

  function checkIfInsuredSigns(user) {
    if (!pdfDetails || !pdfDetails.length) return false;

    const { Placeholders } = pdfDetails[0];
    const agentPlaceholders = Placeholders.filter(
      (placeholder) => placeholder.Role === "Agent"
    );
    const hasMultipleUnsignedSigners =
      unsignedSigners && unsignedSigners.length > 1;
    const isUserAgent = user.Role === "Agent";

    if (agentPlaceholders.length && hasMultipleUnsignedSigners && isUserAgent) {
      setIsAlert({
        isShow: true,
        alertMessage: t("insured-sign-pending-msg")
      });
      return true;
    }
  ...
  const handleSignPad = useCallback(
    async (checkuser) => {
      ...
      let hasSigned = false,
        hasInitials = false,
        isInitialsPosExists = false;

      if (!isAccepted) {
        setOpenTermsModal(true);
        setIsProcessing(false); // Don't forget to reset
        return;
      }

      const currentSigner =
        checkuser ||
        signerPos.find((data) => data.signerObjId === signerObjectId);

      currentSigner.placeHolder.forEach(({ pos }) => {
        pos.forEach(({ type, SignUrl }) => {
          const hasSignUrl = SignUrl !== undefined;
          if (type === "signature" && hasSignUrl) hasSigned = true;
          if (type === "initials") {
            isInitialsPosExists = true;
            if (hasSignUrl) hasInitials = true;
          }
        });
      });
      ...
      if (!hasInitials && isInitialsPosExists) {
        setIsSignPad(true);
        setIsInitial(true);
        setIsProcessing(false);
        return;
      ...
      let tourArr = [];
      let checkIfSignUrlExists = false;
      let checkIfInitialsUrlExists = false;
      let signedObj, signedInitialsObj;

      currentSigner.placeHolder.forEach((placeholder, i) => {
        placeholder.pos.forEach((pos, j) => {
          const hasSignUrl = Object.prototype.hasOwnProperty.call(
            pos,
            "SignUrl"
          );
          if (
            !hasSignUrl &&
            (pos.type == "signature" || pos.type == "initials")
          ) {
            const signKey = pos.key;
            const posIndex = placeholder.pos.findIndex(
              (obj) => obj.key === signKey
            );
            tourArr.push({
              key: signKey,
          ...
          } else if (!checkIfSignUrlExists && pos.type == "signature") {
            signedObj = pos;
            checkIfSignUrlExists = true;
          } else if (!checkIfInitialsUrlExists && pos.type == "initials") {
            signedInitialsObj = pos;
            checkIfInitialsUrlExists = true;
          }
        });
      });
      ...
      try {
        await new Promise((resolve) => {
          setSignerPos((prevState) => {
            ...
            const currentSigner = signerPos.find(
              (data) => data.signerObjId === signerObjectId
            );
            const targetPos =
              currentSigner.placeHolder[tourArr[0].placeHolderIndex].pos[
                tourArr[0].posIndex
              ];

            if (tourArr.length === 1) {
              currentSigner.placeHolder.forEach(({ pos }) => {
                pos.forEach(({ type, options }) => {
                  if (type === "date")
                    options.response = getFormattedLocalTime(true);
                });
              });
            }

            if (targetPos && targetPos.type == "signature") {
              targetPos.SignUrl = signedObj.SignUrl;
              targetPos.Width = signedObj.Width;
              targetPos.Height = signedObj.Height;
              targetPos.options.response = signedObj.SignUrl;
              targetPos.dateAdded = new Date().toISOString();
            }

            if (
              targetPos &&
              targetPos.type == "initials" &&
              signedInitialsObj?.SignUrl
            ) {
              targetPos.SignUrl = signedInitialsObj.SignUrl;
              targetPos.Width = signedInitialsObj.Width;
              targetPos.Height = signedInitialsObj.Height;
              targetPos.options.response = signedInitialsObj.SignUrl;
              targetPos.dateAdded = new Date().toISOString();
            }
        ...
        if (tourArr[1]?.key) {
          setUnSignedWidgetId(tourArr[1].key);
          setPageNumber(tourArr[1].pageNumber);
          setSignKey(tourArr[1].key);
        } else {
          setUnSignedWidgetId("");
    ...
    [isProcessing, isAccepted, signerPos, signerObjectId]
  );

  const checkIfDocNotCancelled = async (isStartBtnClicked = true) => {
    try {
      const docId = docUniqueId.current;
      const status = await Parse.Cloud.run("getDocStatus", { docId });

      if (status === "CANCELLED") {
        setIsAlert({
          isShow: true,
          alertMessage: t("doc-validation-cancelled-msg")
        });
        return;
      }
      ...
      setIsAlert({
        isShow: true,
        alertMessage: t("doc-validation-error-msg")
      });
    }
  };
  ...
  async function embedWidgetsData(
    updatedSignerPos,
    updatedAcceptedTerms = false
  ) {
    if (!isAccepted && !updatedAcceptedTerms) {
      setOpenTermsModal(true);
      return;
    }
    ...
    const signer1OTPRequired = pdfDetails?.[0].Signer1OTPRequired;
    const signer2OTPRequired = pdfDetails?.[0].Signer2OTPRequired;
    const signersArr = pdfDetails?.[0].Signers;
    const url = window.location.pathname;
    const parts = url.split("/");
    const lastUrlValue = parts[parts.length - 1];
    // const isEnableOTP = pdfDetails?.[0]?.IsEnableOTP || false;
    const isEnableOTP =
      signersArr[0].objectId == lastUrlValue
        ? signer1OTPRequired
        : signer2OTPRequired;
    ...
    if (!isEnableOTP || isEmailVerified) {
      try {
        let checkUser = null;
        if (updatedSignerPos) {
          checkUser = updatedSignerPos.filter(
            (data) => data.signerObjId === signerObjectId
          );
        } else {
          checkUser = signerPos.filter(
            (data) => data.signerObjId === signerObjectId
          );
        }
        if (checkUser && checkUser.length > 0) {
          ...
          const isInsurerSignPending = checkIfInsuredSigns(checkUser[0]);
          if (isInsurerSignPending) {
            return;
          }
            ...
            try {
              ...
              if (!signPdfPromise) {
                signPdfPromise = signPdfFun(
                  ...
                  activeMailAdapter,
                  widgets,
                  updatedSignerPos ? updatedSignerPos : signerPos,
                  pdfDetails?.[0].AdditionalUserInfo
                ).finally(() => {
                  // Reset after completion or failure
                  signPdfPromise = null;
                });
              }
              const resSign = await signPdfPromise;
  ...
  const onImageChange = (event) => {
    // Check file type
    if (event.target.files && event.target.files[0].type !== "image/png") {
  ...
  const saveImage = () => {
    ...
    if (
      getUpdatePosition[0].pos[1]?.key &&
      getUpdatePosition[0].pos[1]?.type == "signature" &&
      !getUpdatePosition[0].pos[1]?.SignUrl
    ) {
      setTimeout(() => {
        setUnSignedWidgetId(getUpdatePosition[0].pos[1].key);
        setPageNumber(getUpdatePosition[0].pageNumber);
      }, 400);
    } else if (getUpdatePosition[0].pos[1]?.type == "initials") {
      const updateSignerPos = [...signerPos];
      updateSignerPos.splice(indexofSigner, 1, ...updateSignerData);
      setSignKey(getUpdatePosition[0].pos[1]?.key);
      setTimeout(() => {
        embedWidgetsData(updateSignerPos);
      }, 200);
    } else {
      const updateSignerPos = [...signerPos];
      updateSignerPos.splice(indexofSigner, 1, ...updateSignerData);
  ...
  const saveSign = (type, isDefaultSign, width, height) => {
    ...
    const updateSignerData = currentSigner.map((obj) => {
      if (obj.signerObjId === signerObjectId) {
        // Set current local time for 'date' fields
        getUpdatePosition.forEach(({ pos }) => {
          pos.forEach(({ type, options }) => {
            if (type === "date") {
              options.response = getFormattedLocalTime(true);
            }
    ...
    if (
      ...
      const updateSignerPos = [...signerPos];
      updateSignerPos.splice(index, 1, ...updateSignerData);
      setSignKey(getUpdatePosition[0].pos[1]?.key);
      setTimeout(() => {
        embedWidgetsData(updateSignerPos);
      }, 200);
                ...
                <div className=" w-full md:w-[57%] flex mr-4">
                  ...
                  <div className=" w-full md:w-[95%] ">
                    ...
                    <SignPad
                      ...
                      changeSignVal={changeSignVal}
                      signTypeValue={signTypeValue}
                    />
                    ...
                    <Header
                      ...
                      embedWidgetsData={
                        isPublicTemplate
                          ? handleUserDetails
                          : checkIfDocNotCancelled
                      }
                      isShowHeader={true}
                      setIsDecline={setIsDecline}
                      declineHandler={(obj) => declineHandler(obj)}
                      decline={true}
