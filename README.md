...
- **Two-factor authentication in the product UI:** Account holders can enable authenticator-app based protection from the profile screen, where OpenSign displays enrollment QR codes and validates setup or disable tokens before saving the preference. During subsequent logins the UI automatically prompts for the six-digit code in a dedicated modal before the session continues, keeping the experience consistent with the rest of the app.
